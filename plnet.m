classdef plnet<handle
    ...
    methods
        function obj = plnet(layerSizes, initialWeightRange)
            if nargin == 0
        ...
        function SGD(obj, dataset, nEpochs, eta)
            ...
            if (useValSet == 1)
                ...
                for i1 = 1 : nEpochs
                    ...
                    plot(histTr,'-','Color',[0.1333    0.4863    0.2863],'Linewidth',2)
                    grid on
                    hold on
                    plot(histVa,'--','Color',[0.5,0.3,0.2],'Linewidth',2)
                    legend('Tr','Va')
                    drawnow
                end
            else
